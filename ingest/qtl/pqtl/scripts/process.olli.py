...
def main():
    ...
    # Load data
    # The Olli dataset is problematic because some files have different columns
    # than others ('direction' is missing sometimes, and in other cases column
    # order differs). So we load each file separately and then concatenate.
    storage_client = storage.Client()
    blobs = storage_client.list_blobs('genetics-portal-dev-raw', prefix='protein-gwas/harmonised/OLLI/', delimiter='/')
    file_paths = ['gs://' + b.bucket.name + '/' + b.name for b in blobs]

    dfs = []
    for in_path in file_paths:
        df_temp = load_sumstats(in_path)
        dfs.append(df_temp)
    
    # Take union
    sumstats = reduce(pyspark.sql.DataFrame.unionByName, dfs)
    ...
    sumstats = (
        sumstats.withColumn('study_id', lit(args.study_id))
            ...
            # Don't filter on MAC, since we don't have MAF for Olli dataset
