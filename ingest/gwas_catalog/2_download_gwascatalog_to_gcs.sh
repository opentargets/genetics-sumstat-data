...
cat configs/gwas_metadata_curated.latest.tsv | awk 'BEGIN{FS="\t"} $10 == "1"' | cut -f 1 > configs/ftp_paths.to_download.txt
...
#wget --no-clobber --input-file=../configs/ftp_paths.test.txt --base=ftp://ftp.ebi.ac.uk/pub/databases/gwas/summary_statistics/
...
gsutil -m cp -n *.tsv.gz gs://genetics-portal-dev-raw/gwas_catalog/harmonised_${version_date}/
